...
function updateHorizontalScroll() {
  ...
  // BUG 3: Se divide por window.innerWidth en lugar de window.?
  // Hace que el progreso se calcule mal y el scroll horizontal no funciona bien
  // CORRECCIÓN: const progress = 1 - (rect.top / ??)
...
const words =
  "JavaScript tiene acceso completo al DOM y puede transformar cualquier scroll en una experiencia visual poderosa sin ninguna librería externa".split(
    " ",
  );
...
words.forEach((word, i) => {
  ...
  span.className =
    "reveal-word" +
    (highlightWords.includes(word) ? " highlight-candidate" : "");
...
// BUG 4: threshold es 0 en lugar de ??
// El observer dispara cuando el elemento apenas toca el viewport (0%),
// no cuando está 30% visible — los contadores arrancan demasiado pronto
// CORRECCIÓN: threshold:??
...
function updateHeroParallax() {
  ...
  if (scrollY < window.innerHeight) {
    // BUG 5: El factor de parallax es 1 en lugar de ?
    // El título se desplaza a la misma velocidad que el scroll,
    // desapareciendo instantáneamente al bajar en lugar de moverse con suavidad
    // CORRECCIÓN: Cambiar * 1 por * ?
    heroTitle.style.transform = `translateY(${scrollY * 0.4}px)`;
  }
}

// EVENTO SCROLL PRINCIPAL
